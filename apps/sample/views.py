...
from apps.core.roles import ROLE_RECEPTIONIST, ROLE_MANAGER, ROLE_ADMIN, ROLE_SUPER_ADMIN, ROLE_GUEST
...
class DashboardOverviewPage(RequireAnyRoleMixin, TemplateView):
    ...
    template_name = "dashboard/overview.html"  # default (admin)

    # -----------------------------
    # ROLE ROUTER
    # -----------------------------
    def get(self, request, *args, **kwargs):
        if request.user.role == ROLE_GUEST:
            self.template_name = "dashboard/guest_overview.html"
            self.mode = "guest"
        else:
            self.template_name = "dashboard/overview.html"
            self.mode = "admin"
    ...
    def get_context_data(self, **kwargs):
        ...
        if getattr(self, "mode", "admin") == "guest":
            return self._guest_context(ctx)

        return self._admin_context(ctx)

    # =========================================================
    # ADMIN DASHBOARD LOGIC
    # =========================================================
    def _admin_context(self, ctx):
        today = date.today()
        ...
        available_rooms = Room.objects.exclude(
            id__in=occupied_room_ids_today
        ).count()

        total_revenue = (
            Booking.objects.aggregate(total=Sum("payment_amount"))["total"] or 0
        )
        ...
        chart_labels = [t["day"].strftime("%b %d") for t in trend]
        ...
        cat_labels = [r["room__category__name"] or "—" for r in rev_by_cat]
        ...
        def room_floor(room_number):
            m = re.search(r"\d+", str(room_number or ""))
        ...
        for r in Room.objects.select_related("category").order_by("room_number"):
            ...
            floor_groups[room_floor(r.room_number)].append({
                "id": r.id,
        ...
        floor_rooms = [
            {"floor": f or "—", "rooms": rooms}
            for f, rooms in sorted(floor_groups.items())
        ]
    ...
    # =========================================================
    def _guest_context(self, ctx):
        guest = getattr(self.request.user, "guest_profile", None)
...
@method_decorator(login_required, name="dispatch")
class GuestDashboardOverviewPage(RequireAnyRoleMixin, TemplateView):
    template_name = "dashboard/overview.html"
    allowed_roles = (ROLE_ADMIN, ROLE_SUPER_ADMIN, ROLE_GUEST)
